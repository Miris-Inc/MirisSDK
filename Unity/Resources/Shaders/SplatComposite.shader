...
//
// Draws each view's Shark surface straight over the camera's buffer, one texel per pixel. In mono
// there is one view and only _LeftTex is bound, which is what eye index 0 samples.
//
...
Shader "Miris/SplatComposite"
{
    Properties
    {
        ...
        // 1 flips V when sampling, 0 does not. See SplatRenderer.CompositeVFlip.
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct v2f
            {
                ...
                float2 uv : TEXCOORD0;
                // Camera-convention NDC - the domain _Reproj* operates on. See vert.
            ...
            sampler2D _RightTex;
            float _VFlip;
            // Not in Properties: ShaderLab has no Matrix type, and one there fails to compile.
            // Bound by Material.SetMatrix from SharkSplatRenderSystem.
            float4x4 _ReprojLeft;
            float4x4 _ReprojRight;
            ...
            v2f vert(appdata v)
            {
                ...
                // NOT o.pos.xy: that carries the _ProjectionParams.x render-target flip, which is
                // not part of the camera's projection. Undoing it leaves camera NDC = v.vertex.xy,
                // which is the convention Camera.projectionMatrix (and so _Reproj*) is in.
                o.ndc = v.vertex.xy;
                return o;
            }
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                // Where this pixel's ray pointed when Shark rasterized this surface. The z is
                // arbitrary: the warp has translation stripped, so every point on a ray reprojects
                // identically and there is no scene-distance term.
                float4x4 reproj = unity_StereoEyeIndex == 0 ? _ReprojLeft : _ReprojRight;
                float4 clipRender = mul(reproj, float4(i.ndc, 0.0, 1.0));

                // Ray fell behind the render-time eye; dividing would fold it back in as a
                // mirrored ghost.
                if (clipRender.w <= 0.0)
                {
                ...
                float2 uv = clipRender.xy / clipRender.w * 0.5 + 0.5;
                uv.y = lerp(uv.y, 1.0 - uv.y, _VFlip);

                // The warp reaches outside the rendered frustum at the edge turned toward. No data
                // there, and clamping would smear edge texels across the band, so fade out instead.
                // saturate of a negative excess is exactly 0, so an identity warp leaves no seam.
                float2 excess = max(-uv, uv - 1.0);
                float mask = 1.0 - saturate(max(excess.x, excess.y) * 512.0);

                fixed4 colour = unity_StereoEyeIndex == 0 ? tex2D(_LeftTex, uv) : tex2D(_RightTex, uv);

                // All four channels: the surface is premultiplied (see the Blend note), so colour
                // and alpha fall off together.
                return colour * mask;
            }
            ENDCG
        }
