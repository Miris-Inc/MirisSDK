...
namespace Miris.Tests
{
    public class MirisTestUtils
    {
        ...
        /// <param name="port">The server port, or 0 to use AQUA_TEST_SERVER_PORT.</param>
        static public string BuildDirectContentUrl(string contentPath, int port = 0)
        {
            if (port == 0)
            {
                string envPort = System.Environment.GetEnvironmentVariable("AQUA_TEST_SERVER_PORT");
                port = !string.IsNullOrEmpty(envPort) && int.TryParse(envPort, out int parsed) ? parsed : 4615;
            }
            string host = GetTestServerHost();
            return $"http://{host}:{port}/content/{contentPath}";
        }

        static public ContentManifest LoadContentManifest(int port = 0)
        {
            string url = BuildDirectContentUrl("conditioned/manifest.json", port);
            try
            {
                using (var client = new System.Net.Http.HttpClient())
                {
                    ...
                    return JsonUtility.FromJson<ContentManifest>(json);
                }
            }
            catch (System.Exception ex)
            {
                UnityEngine.Debug.LogWarning($"[MirisTestUtils] No content manifest at {url}: {ex.Message}");
                return null;
            }
        }

        static public string GetFirstAssetContentPath()
        {
            var manifest = LoadContentManifest();
            if (manifest != null && manifest.assets != null && manifest.assets.Length > 0)
                return manifest.assets[0].content_path;
            return null;
        }
    }
    ...
    {
        public ContentAssetEntry[] assets;
        public int count;
    }

    [System.Serializable]
    public class ContentAssetEntry
    {
        public string name;
        public string variant;
        public string content_path;
        public string content_dir;
        public string drop_path;
    }
