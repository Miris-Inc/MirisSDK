...
namespace Miris.Tests
{
    public class TestRenderer : RendererTestBase {
        ...
        private static ContentManifest s_manifest;
        private static bool s_manifestLoaded;

        private static ContentManifest GetManifest()
        {
            if (!s_manifestLoaded)
            {
                s_manifest = MirisTestUtils.LoadContentManifest();
                s_manifestLoaded = true;
                if (s_manifest != null)
                    UnityEngine.Debug.Log($"[TestRenderer] Content manifest: {s_manifest.count} assets");
                else
                    UnityEngine.Debug.LogWarning("[TestRenderer] No content manifest available");
            }
            return s_manifest;
        }
        ...
        public IEnumerator TestGeometryPath()
        {
            ...
            if (manifest?.assets != null)
            {
                ...
                {
                    if (!string.IsNullOrEmpty(entry.drop_path))
                    {
                        dropPath = entry.drop_path;
                        break;
                    }
                }
            }
            Assert.IsNotNull(dropPath, "No .drop file found in conditioned content");

            stream.gameObject.transform.localScale = new Vector3(3, 3, 3);
            stream.m_url = MirisTestUtils.BuildDirectContentUrl(dropPath);
        ...
        {
            // Setup scene
            CreateMirisStream(out MirisStreamController streamController, out MirisStreamTest stream);
            SetupCamera(stream.gameObject, out Camera camera, out GraphicsTestSettings graphicsTestSettings);
