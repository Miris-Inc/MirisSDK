...
namespace Miris.Editor
{
    public class MirisReleaseDownloader : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            using (new EditorGUILayout.VerticalScope("box"))
            {
                EditorGUILayout.LabelField("Loaded Library (Editor)", EditorStyles.boldLabel);
                try
                {
                    string libVersion = Miris.Runtime.MirisApi.GetLibAquaVersionString();
                    EditorGUILayout.LabelField($"Library Version: {libVersion}", EditorStyles.miniLabel);
                }
                catch (Exception ex)
                {
                    EditorGUILayout.HelpBox($"Error retrieving library version: {ex.Message}", MessageType.Error);
                }
            }
