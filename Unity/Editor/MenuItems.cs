...
namespace Miris.Editor.Tools
{
    ...
    public static class MenuItems
    {
        ...
        private static void AutoConfigureXRSimulation()
        {
            if (IsCIMode())
            {
                // Don't enable XR Simulation for device builds - they use real XR runtime on device
                if (IsDeviceBuild())
                {
                    Debug.Log("[Miris] Skipping XR Simulation for device build - will use real XR runtime on device");
                    return;
                }

                // Editor PlayMode tests need XR simulation
                SetXRSimulation(true);
                Debug.Log("[Miris] Auto-enabled XR Simulation for CI/batch mode Editor testing");
            }
            // Interactive development - leave current state, developers can toggle manually
        }
        private static bool IsDeviceBuild()
        {
            var buildTarget = EditorUserBuildSettings.activeBuildTarget;
            bool isDevice = buildTarget == BuildTarget.Android ||
                           buildTarget == BuildTarget.iOS ||
                           buildTarget == BuildTarget.VisionOS;

            if (isDevice)
            {
                Debug.Log($"[Miris] Detected device build target: {buildTarget}");
            }
