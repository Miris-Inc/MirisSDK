...
namespace Miris.Runtime
{
    ...
    public class MirisStream : MonoBehaviour
    {
        ...
        public Status GetStatus()
        {
            ...
            // If prefetching, then it is legitimate to have no renderable data ready despite the stream content having been fetched (or still being fetched)
            if (m_streamController.GetStreamFetchMode(this) == StreamFetchMode.Prefetch)
            {
                return Status.Streaming;
            }
        ...
        {
            if (!IsLoaded() || m_firedLoadedActions
                || (m_defaultCameraObject != null && m_viewingVolumeObject != null))
            {
                return;
            }
            ...
            if (m_defaultCameraObject == null)
            {
                ...
                {
                    m_defaultCameraObject = new SceneObject(client, defaultCameraId);
                }
            }
            ...
            if (m_viewingVolumeObject == null)
            {
                ...
                if (viewingVolumeId >= 0)
                {
                    m_viewingVolumeObject = new SceneObject(client, viewingVolumeId);
                }
            }
        }
