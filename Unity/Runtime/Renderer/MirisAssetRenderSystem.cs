...
namespace Miris.Runtime
{
    ...
    public class MirisAssetRenderSystem
    {
        ...
        private void CreateSystemResources() {
            ...
            // THREADS_PER_GROUP in MirisThreadGroupSize.hlsl - what the kernels are compiled
            // at unless MIRIS_SMALL_THREADGROUPS shrinks them to 256.
            const int FullThreadGroupSize = 1024;
            const string SmallThreadGroupsKeyword = "MIRIS_SMALL_THREADGROUPS";

            // Global, and set before any compute shader is dispatched. A device that will not run
            // a 1024-thread kernel reports it as invalid rather than clamping it, so every
            // dispatch silently does nothing - the visionOS simulator caps threadgroups at 256.
            //
            // Driven by the reported limit and not a build-time define.
            int maxWorkGroupSize = SystemInfo.maxComputeWorkGroupSize;
            if (maxWorkGroupSize > 0 && maxWorkGroupSize < FullThreadGroupSize)
            {
                Shader.EnableKeyword(SmallThreadGroupsKeyword);
                MirisDebug.Log($"Enabled {SmallThreadGroupsKeyword}: this device reports a maximum "
                               + $"compute work group size of {maxWorkGroupSize}, below the "
                               + $"{FullThreadGroupSize} the kernels are compiled at");
            }
            else
            {
                Shader.DisableKeyword(SmallThreadGroupsKeyword);
            }
        ...
        public void ProcessComponents(Camera camera)
        {
            using (s_gatherAndSortMarker.Auto())
            {
                ...
                m_activeComponents.Sort((a, b) =>
                {
                    ...
                    // Transform the bound centers to worldspace. objectCenterA/B are already
                    // stream-relative (see MirisAssetDataSource.GetObjectBounds), so only the
                    // stream's own transform is applied here - m_transform would double up the
                    // asset-relative part that's already baked into the bounds.
                    Vector3 worldCenterA = a.m_streamTransform.TransformPoint(objectCenterA);
                    Vector3 worldCenterB = b.m_streamTransform.TransformPoint(objectCenterB);
        ...
        private void RefreshXrRenderState(Camera camera)
        {
            ...
            if (hasValidDescriptor)
            {
                ...
                m_xrTextureDescriptor.graphicsFormat = GraphicsFormat.R16G16B16A16_SFloat;
            }
        ...
        private void RenderUsingBuiltinPipeline(Camera camera, CommandBuffer commandBuffer)
        {
            ...
            using (s_renderMarker.Auto())
            {
                ...
                {
                    m_commandBuffer.GetTemporaryRT(ShaderIds.MirisAssetRT, -1, -1, 0, FilterMode.Point, GraphicsFormat.R16G16B16A16_SFloat);
                }
