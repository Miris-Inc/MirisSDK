...
namespace Miris.Runtime
{
    ...
    public class MirisAssetRenderSystem
    {
        ...
        private void CreateSystemResources() {
            ...
            // Global, and set before any compute shader is dispatched: the simulator reports a
            // 1024-thread kernel as invalid rather than clamping it, so every dispatch would
            // silently do nothing. See MirisThreadGroupSize.hlsl.
            Shader.EnableKeyword("MIRIS_SMALL_THREADGROUPS");
            MirisDebug.Log("Enabled MIRIS_SMALL_THREADGROUPS for the visionOS simulator");
#endif
