...
namespace Miris.Runtime
{
    ...
    public static class SplatRendererBridge
    {
        ...
        // 1 real, 0 stub. Callers want SharkSplatRenderSystem.IsSupported, which caches this.
        [DllImport(MirisApi.AquaUnityPath)]
        static public extern int AquaSplatRenderer_IsAvailable();
        ...
        // One model root's opacity, 0 to 1, multiplied into every one of its splats' alpha. Exactly
        // 0 drops it from the render ordering rather than rasterizing it invisibly, so a hidden
        // stream costs nothing to draw while staying resident. Stages only, like SetModelTransform.
        [DllImport(MirisApi.AquaUnityPath)]
        static public extern void AquaSplatRenderer_SetModelOpacity(int renderer, int modelRootObjectId, float opacity);
        ...
        // The camera a slot was RENDERED with, column-major, for reprojecting it to the current
        // pose. 0 on success; -1 means that slot has not been rendered yet. See the header.
        [DllImport(MirisApi.AquaUnityPath)]
        static public extern int AquaSplatRenderer_GetSlotPose(int renderer, int viewIndex, int slot, float[] view,
                                                               float[] proj);
