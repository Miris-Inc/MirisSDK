...
{
    /// <summary>
    /// MirisAssetDataSource provides data to a MirisAssetRenderer.
    /// </summary>
    public class MirisAssetDataSource
    {
        ...
        private AttributeBuffer[] m_semanticBuffers = new AttributeBuffer[(int) AttributeSemantic.MaxAttributeSemantic];
        ...
        public MirisAssetDataSource()
        {
            // Init cache with empty values
            Array.Fill(m_semanticBuffers, null);
        }
        ...
        {
            // Is it cached?
            int cacheIndex = (int) semantic;
            Debug.Assert(cacheIndex < (int) AttributeSemantic.MaxAttributeSemantic, "Invalid AttributeSemantic specified for AttributeBuffer");
            if (!m_dirty && m_semanticBuffers[cacheIndex] != null)
            {
                return m_semanticBuffers[cacheIndex];
            }
            ...
            if (attributeInfo.m_mosaicDescriptorCount > 0)
            {
                ...
                m_semanticBuffers[cacheIndex] = ab;
                return ab;
            }
            ...
            m_semanticBuffers[cacheIndex] = attributeBuffer;
        ...
        unsafe private void TryAssociateBlockBoundsBuffer(AttributeBuffer attributeBuffer, string bufferName, AttributeInfo attributeInfo)
        {
            ...
            catch (Exception ex)
            {
                ...
                MirisDebug.Log($"[MirisAssetDataSource] Block bounds not available for attribute '{blockBoundsName}': {ex.Message}");
            }
        }
    }
}
