...
namespace Miris.Runtime
{
    ...
    public class GeometryRenderer : MirisAssetRenderer
    {
            ...
            if (!m_disposed)
            {
                if (disposing)
                {
                    using (s_destroyResourcesMarker.Auto())
                    {
                        ...
                        GameObject.DestroyImmediate(m_depthComputeShader);
                        m_depthComputeShader = null;
                        GameObject.DestroyImmediate(m_map3DGSShader);
                        m_map3DGSShader = null;
                    }
                }
        ...
        private void SortSplats(CommandBuffer commandBuffer, Camera camera, MirisTransform transform)
        {
            bool doSort = false;
        ...
        private void GPUMapSplats(CommandBuffer commandBuffer, Camera camera, MirisTransform transform, bool shOnly, ComputeBuffer eyeDataBuffer, int firstSplat, int splatsToMapCount, ComputeBuffer gpuSplats) {
            Dispatch3DGSCalculationKernel(commandBuffer, camera, transform, shOnly, eyeDataBuffer, firstSplat, splatsToMapCount, gpuSplats);
        }

        private void DrawSplats(CommandBuffer commandBuffer, MirisTransform transform, int splatsToDrawCount, ComputeBuffer gpuSplatsBuffer, bool isSinglePassXR)
        {
        ...
        private void Draw(CommandBuffer commandBuffer, MirisTransform transform, ShaderPassId shaderPassId, int splatsToDrawCount, ComputeBuffer gpuSplatsBuffer, bool isSinglePassXR)
        {
