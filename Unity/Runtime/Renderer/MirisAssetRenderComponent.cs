...
namespace Miris.Runtime
{
    ...
    public class MirisAssetRenderComponent : IDisposable
    {
        ...
        private MirisAssetDataSource[] m_prevDataSources;
        private MirisAssetDataSource[] m_validDataSources;
        private MirisAssetDataSource[] m_prevValidDataSources;
        // Base Renderer instance
        private MirisAssetRenderer m_gsRenderer;
        ...
        internal MirisTransform m_streamTransform;
        ...
        // Whole-asset opacity in [0, 1]. Honoured by the Geometry pipeline only: it rides the
        // _DataSourceOpacity buffer, which PointRender.compute never samples.
        //
        // Driven by MirisStream every frame, so assigning it from application code would silently
        // revert -- hence the internal setter. Set MirisStream.Opacity instead.
        ...
        static string s_profilerPrefix = "[MirisAssetRenderComponent] ";
        ...
        // MirisAssetRenderSystem draws these splats - Shark, through SharkSplatRenderSystem - in
        // which case this component must do neither of the two things it otherwise does: register
        // with the system (which would draw the same splats a second time) or build its GPU renderer
        // (which would cost VRAM for an asset already resident in the other renderer).

        private bool m_suppressRendering;

        // True once the component has gone fully transparent and released its GPU renderer. This
        // latches the release: setting it before the release happens would skip it entirely.
        private bool m_releasedByOpacity;

        // Frame this component first read as fully transparent, or -1 when it is visible.
        private int m_invisibleSinceFrame = -1;

        // # of frames a component must stay transparent before its renderer is released. Covers GPU
        // lag rather than indecision
        private const int c_invisibleFramesBeforeRelease = 10;

        private bool RenderingSuppressed => m_suppressRendering || m_releasedByOpacity;
        ...
        public MirisAssetRenderComponent(bool suppressRendering = false)
        {
            m_suppressRendering = suppressRendering;
            if (!m_suppressRendering)
            {
                MirisAssetRenderSystem.m_instance.RegisterRenderer(this);
            }
        }
        ...
        internal void SetRenderingSuppressed(bool suppressed)
        {
            ...
            if (suppressed)
            {
                MirisAssetRenderSystem.m_instance.UnregisterRenderer(this);
            }
            else
            {
                MirisAssetRenderSystem.m_instance.RegisterRenderer(this);
        ...
        public void Update(Transform transform)
        {
            ...
            m_streamTransform = new MirisTransform(transform); // TODO: Maybe make this a struct so we don't keep reallocing?
            m_transform = m_streamTransform * m_assetMatrix;

            UpdateOpacitySuppression();
            ...
            if (RenderingSuppressed)
            {
                ...
                //
                // We still need to keep bounds up to date, including when an existing data
                // source's ancestor-relative bounds went stale from a transform change rather
                // than the data-sources array itself being swapped.
            ...
            m_gsRenderer.SetAssetOpacity(Opacity);
        ...
        private void UpdateOpacitySuppression()
        {
            if (Opacity > 0.0f)
            {
                m_invisibleSinceFrame = -1;
                m_releasedByOpacity = false;
                return;
            }

            if (m_releasedByOpacity)
            {
                return;
            }
            ...
            if (m_gsRenderer == null)
            {
                m_releasedByOpacity = true;
                return;
            }

            if (m_invisibleSinceFrame < 0)
            {
                m_invisibleSinceFrame = Time.frameCount;
            }

            if (Time.frameCount - m_invisibleSinceFrame < c_invisibleFramesBeforeRelease)
            {
                return;
            }
            ...
            // Returning the memory is the whole point; holding the object would hold the atlas.
            m_gsRenderer.Dispose();
            m_gsRenderer = null;
        }
        ...
        private MirisAssetDataSource[] GetVisibleDataSourcesSorted(Camera camera, CullingState cullingState=CullingState.Enabled)
        {
            s_getVisibleDataSourcesMarker.Begin();

            List<(MirisAssetDataSource dataSource, float distance)> visibleDataSources = new();
            ...
            if (cullingState == CullingState.Enabled)
            {
                ...
                foreach (Plane plane in frustumPlanes)
                {
                    ...
                    Vector3 planeCentreLocal = m_streamTransform.InverseTransformPoint(planeCentreWorld);
                    Vector3 planeNormalLocal = m_streamTransform.InverseTransformDirection(frustumPlanes[planeIndex].normal);
            ...
            foreach (var dataSource in m_dataSources)
            {
                ...
                {
                    // Same stream-relative frame as m_cullingPlanes above.
                    float distance = Vector3.Distance(camWorldPos, m_streamTransform.TransformPoint(dataSourceBounds.center));
                    visibleDataSources.Add((dataSource, distance));
                }
            }

            visibleDataSources.Sort((a, b) => a.distance.CompareTo(b.distance));
            MirisAssetDataSource[] sorted = visibleDataSources.Select(x => x.dataSource).ToArray();
        ...
        {
            m_objectBounds = m_dataSourceBounds;

            Vector3[] worldCorners = BoundsUtils.BoundsGetCorners(m_dataWorldBounds);
            m_worldBounds = new Bounds(m_streamTransform.TransformPoint(worldCorners[0]), Vector3.zero);
            for (int cornerIndex = 1; cornerIndex < worldCorners.Length; cornerIndex++)
            {
                m_worldBounds.Encapsulate(m_streamTransform.TransformPoint(worldCorners[cornerIndex]));
            }
        }
        ...
        // bounds from them.
        private void UpdateBounds(MirisAssetDataSource[] objectSources, MirisAssetDataSource[] worldSources)
        {
        ...
        {
            foreach (MirisAssetDataSource dataSource in m_validDataSources)
            {
