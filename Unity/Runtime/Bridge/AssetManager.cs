...
namespace Miris.Runtime
{
    ...
    public class AssetManager : IDisposable
    {
        ...
        /// <summary>
        /// Get available assets from the server environment, filtered by the tags set with
        /// <see cref="SetTags"/>, together with pagination cursors.
        ...
        public Task<AssetsResultPage> GetAssets(int limit = 0, string cursor = "", PageDirection direction = PageDirection.None)
        {
            ...
            return GetAssetsWithTags(tagStrings, limit, cursor, direction);
        }

        /// <summary>
        /// Like <see cref="GetAssets(int, string, PageDirection)"/>, but filtered by all of <paramref name="tags"/>
        /// instead of the tags from <see cref="SetTags"/>, which are left untouched. Null or empty means no filter.
        /// </summary>
        public Task<AssetsResultPage> GetAssets(IEnumerable<string> tags, int limit = 0, string cursor = "",
            PageDirection direction = PageDirection.None)
        {
            var tagStrings = tags?.Where(tag => !string.IsNullOrWhiteSpace(tag)).ToArray() ?? Array.Empty<string>();
            return GetAssetsWithTags(tagStrings, limit, cursor, direction);
        }

        private Task<AssetsResultPage> GetAssetsWithTags(string[] tagStrings, int limit, string cursor, PageDirection direction)
        {
