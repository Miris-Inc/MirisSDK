...
namespace Miris.Runtime
{
    public class AssetManager : IDisposable
    {
        ...
        private string m_defaultViewerKey = "";
        ...
        public Task<string[]> GetAvailableEnvironments()
        {
            ...
            return Task.Run(() =>
            {
                using (var result = client.GetAvailableEnvironments())
                {
                    ...
                    for (int i = 0; i < (int)result.Count; i++)
                    {
                        environments[i] = result[i].m_name;
                    }
                    return environments;
                }
            });
        ...
        /// <summary>
        /// Switch ENVs. Since viewer keys are invalid outside of their bound ENV, a viewer key for the target ENV
        /// should be specified.
        /// </summary>
        public async Task<bool> SetServerEnvironment(string environment, string assetViewerKey)
        {
            ...
            if (result)
            {
                ...
                m_client.SetAssetViewerKey(assetViewerKey);
        ...
        public void SetDefaultViewerKey(string viewerKey)
        {
            m_defaultViewerKey = viewerKey;
        }

        public Task<bool> ResetEnvironmentToDefault()
        {
            return SetServerEnvironment(GetDefaultEnvironment(), m_defaultViewerKey);
        }
    }
}
