...
// TODO: restore from ./annotation/types when re-enabling annotation
// import type { ReconciledDetection3D, ReconciledPolyline3D } from "./annotation/types";
export type OverlayLabel = { _id: string; path: string; selected: boolean; color?: string; label?: string; sampleId?: string; _cls: string; instance?: { _cls: "Instance"; _id: string }; } & Record<string, unknown>;
type CuboidTransformData = { location: [number, number, number]; dimensions: [number, number, number]; rotation?: [number, number, number]; quaternion?: [number, number, number, number]; };
type PolylineTransformData = { points3d: [number, number, number][][]; closed?: boolean; };
type ReconciledLabelBase3D = { isNew?: boolean; label?: string };
export type ReconciledDetection3D = Omit<OverlayLabel, "selected"> & CuboidTransformData & ReconciledLabelBase3D & { _cls: "Detection"; _id: string; path: string; } & Record<string, unknown>;
export type ReconciledPolyline3D = Omit<OverlayLabel, "selected"> & ReconciledLabelBase3D & PolylineTransformData & { _cls: "Polyline"; _id: string; path: string; closed?: boolean; } & Record<string, unknown>;
import type {
  ACTION_SET_PCDS,
...
// TODO: restore from ./labels/loader when re-enabling labels: import type { OverlayLabel } from "./labels/loader";
