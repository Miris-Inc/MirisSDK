...
// TODO: restore import from ../annotation/types when re-enabling annotation
// import type { AnnotationPlaneState, SegmentState, SelectedPoint, TransformMode } from "../annotation/types";
type TransformMode = "translate" | "rotate" | "scale";
interface SelectedPoint { labelId: string; segmentIndex: number; pointIndex: number; }
interface SegmentState { isActive: boolean; isClosed: boolean; currentMousePosition: [number, number, number] | null; vertices: [number, number, number][]; }
interface AnnotationPlaneState { enabled: boolean; position: [number, number, number]; quaternion: [number, number, number, number]; showX: boolean; showY: boolean; showZ: boolean; }
import {
  ANNOTATION_CUBOID,
  ANNOTATION_POLYLINE,
  SHADE_BY_HEIGHT,
...
export const liveDetectionsAtom = atom<OverlayLabel[]>({
  key: "miris-liveDetections",
  default: [],
});
