...
export function useFetchFrustumParameters() {
  ...
  // const { resolveUrlForImageSlice, isLoadingImageSlices } = useImageSlicesIfAvailable(modalSample);
  const resolveUrlForImageSlice = (_sliceName: string): string | undefined => undefined;
  const isLoadingImageSlices = false;
