...
export const useFo3d = (sample: fos.ModalSample): UseFo3dReturnType => {
  ...
  const mirisAssetUuid: string = sample.sample.miris_asset_uuid ?? "";
  const dataset = useRecoilValue(fos.dataset);
  const mirisViewerKey = dataset?.info["miris_viewer_key"] ?? DEFAULT_VIEWER_KEY;
  const isMirisStream = mirisAssetUuid !== "" && mirisViewerKey !== "";

  const [isLoading, setIsLoading] = useState(true);
  const [loadError, setLoadError] = useState<Error | null>(null);
  const [rawData, setRawData] = useState<FiftyoneSceneRawJson | null>(null);
  let fo3dRoot: string | null = null;
  ...
  if (!isMirisStream) {
    ...
    useEffect(() => {
      // Manufacture our own fo3d data with the miris stream
      const fakeFo3dJson = {
        "_type": "Scene",
        "name": `Scene-MirisStream-${mirisAssetUuid}`,
        "visible": true,
        "position": [
            0.0,
            0.0,
        ...
        "children": [
          {
            "_type": "mirisstream",
            name: `MirisStream-${mirisAssetUuid}`,
            visible: true,
            viewerKey: mirisViewerKey,
            streamUuid: mirisAssetUuid,
            // Apply x-axis rotation of 90 degrees to align with Miris's "up" orientation
            quaternion: [
              0,
              0,
              0,
      ...
      setRawData(fakeFo3dJson);
      setLoadError(null);
      setIsLoading(false);
    }, [mirisAssetUuid, mirisViewerKey]);
  }
