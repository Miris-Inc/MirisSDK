...
namespace Aqua.Editor.Tools
{
    ...
    /// </summary>
    public static class AquaMenuItems
    {
        private const string XR_SIM_MENU_PATH = "Aqua/Toggle XR Simulation";
        ...
        private static void AutoConfigureXRSimulation()
        {
            if (IsCIMode())
            {
                ...
                Debug.Log("[Aqua] Auto-enabled XR Simulation for CI/batch mode testing");
            }
        ...
        public static void ToggleXRSimulation()
        {
            ...
            Debug.Log($"[Aqua] XR Simulation {status} - Now in {context}");
        ...
        private static bool IsXRSimulationEnabled()
        {
                        ...
                        if (managerInstance != null)
                        {
                            ...
                            if (activeLoadersProperty != null)
                            {
                                ...
                                if (activeLoaders != null)
                                {
                                    ...
                                    Debug.Log($"[Aqua] IsXRSimulationEnabled() - Found simulation loader: {foundSimulation}");
                                    
                                    // Sync EditorPrefs with actual state
                                    EditorPrefs.SetBool("Aqua.XRSimulation.Enabled", foundSimulation);
            ...
            {
                Debug.LogWarning($"[Aqua] Error checking XR Simulation state: {e.Message}");
            }
            
            // Fallback to EditorPrefs for XR Simulation state
            bool editorPrefsValue = EditorPrefs.GetBool("Aqua.XRSimulation.Enabled", false);
            Debug.Log($"[Aqua] IsXRSimulationEnabled() - Fallback to EditorPrefs: {editorPrefsValue}");
        ...
        private static bool IsCIMode()
        {
            ...
            {
                Debug.Log("[Aqua] Detected batch mode - CI environment");
                return true;
            }
            ...
            {
                Debug.Log("[Aqua] Detected GITHUB_ACTIONS environment variable - CI environment");
                return true;
            }
            ...
            {
                Debug.Log("[Aqua] Detected CI environment variable - CI environment");
                return true;
            }
            ...
            {
                Debug.Log("[Aqua] Detected AQUA_CI_MODE environment variable - CI environment");
                return true;
            }
        ...
        private static void SetXRSimulation(bool enabled)
        {
            try
            {
                    ...
                    if (generalSettingsInstance != null)
                    {
                        ...
                        if (managerInstance != null)
                        {
                            ...
                            if (xrSimulationLoaderType != null)
                            {
                                ...
                                Debug.Log($"[Aqua] XR Simulation currently active: {isCurrentlyActive}, trying to set to: {enabled}");
                                    ...
                                    foreach (var method in allMethods)
                                    {
                                        ...
                                        try
                                        {
                                            if (parameters.Length == 1)
                                            {
                                                ...
                                                {
                                                    Debug.Log($"[Aqua] Trying method {method.Name} with loader instance parameter");
                                                ...
                                                {
                                                    Debug.Log($"[Aqua] Trying method {method.Name} with Type parameter");
                                        ...
                                        {
                                            Debug.LogWarning($"[Aqua] Method {method.Name} failed: {ex.Message}");
                                        }
                                    }
                                    
                                    if (success)
                                    {
                                        Debug.Log("[Aqua] XR Simulation enabled in XR Plug-in Management");
                                        EditorPrefs.SetBool("Aqua.XRSimulation.Enabled", true);
                                    }
                                    else
                                    {
                                        Debug.LogWarning("[Aqua] Failed to add XR Simulation loader with all available methods");
                                        // Manual addition as fallback
                                        activeLoaders.Add(simulationLoader);
                                        Debug.Log("[Aqua] XR Simulation manually added to active loaders");
                                        EditorPrefs.SetBool("Aqua.XRSimulation.Enabled", true);
                                    }
                                }
                                else if (enabled && isCurrentlyActive)
                                {
                                    Debug.Log("[Aqua] XR Simulation already enabled");
                                    EditorPrefs.SetBool("Aqua.XRSimulation.Enabled", true);
                                }
                                else
                                {
                                    ...
                                    if (tryRemoveLoaderMethod != null)
                                    {
                                        ...
                                        catch
                                        {
                                            ...
                                            {
                                                Debug.LogWarning($"[Aqua] Remove loader failed: {ex.Message}");
                                            }
                                        }
                                    }
                                    ...
                                    if (success)
                                    {
                                        Debug.Log("[Aqua] XR Simulation disabled in XR Plug-in Management");
                                        EditorPrefs.SetBool("Aqua.XRSimulation.Enabled", false);
                                    }
                                    else
                                    {
                                        Debug.LogWarning("[Aqua] Failed to remove XR Simulation loader - trying manual removal");
                                        ...
                                        if (activeLoaders != null)
                                        {
                                            ...
                                            for (int i = activeLoaders.Count - 1; i >= 0; i--)
                                            {
                                                ...
                                                {
                                                    Debug.Log($"[Aqua] Manually removing XR Simulation loader at index {i}");
                                            ...
                                            if (removedCount > 0)
                                            {
                                                Debug.Log($"[Aqua] XR Simulation manually removed from active loaders ({removedCount} instances)");
                                                EditorPrefs.SetBool("Aqua.XRSimulation.Enabled", false);
                                                success = true;
                                            }
                                            else
                                            {
                                                Debug.LogWarning("[Aqua] No XR Simulation loaders found in active loaders for manual removal");
                                            }
                                        }
                                    }
                                }
                            ...
                            {
                                Debug.LogWarning("[Aqua] XR Simulation loader not found - make sure XR Interaction Toolkit is installed");
                                // Fallback to EditorPrefs tracking
                                EditorPrefs.SetBool("Aqua.XRSimulation.Enabled", enabled);
                            }
                        }
                    }
                }
                ...
                {
                    Debug.LogWarning("[Aqua] XR Management types not found - using preference tracking only");
                    EditorPrefs.SetBool("Aqua.XRSimulation.Enabled", enabled);
                }
            ...
            {
                Debug.LogError($"[Aqua] Failed to toggle XR Simulation: {e.Message}");
                // Fallback to EditorPrefs tracking
                EditorPrefs.SetBool("Aqua.XRSimulation.Enabled", enabled);
            }
        }
        ...
        private static System.Type GetXRSimulationLoaderType()
        {
                ...
                foreach (var assembly in assemblies)
                {
                    if (assembly.FullName.Contains("XR.Simulation") || assembly.FullName.Contains("Simulation"))
                    {
                        foreach (var type in assembly.GetTypes())
                        {
                            ...
                            {
                                Debug.Log($"[Aqua] Found potential XR Simulation loader: {type.FullName}");
                                return type;
                            }
                        }
                    }
            ...
            {
                Debug.LogWarning($"[Aqua] Error searching for XR Simulation loader: {e.Message}");
            }
