...
namespace Aqua.Runtime
{
    ...
    public class GaussianSplatRenderComponent : MonoBehaviour
    {
        ...
        [SerializeField]
        [Tooltip("The asset to render")]
        public GaussianSplatAquaDataSource[] m_dataSources;
        private GaussianSplatAquaDataSource[] m_prevDataSources;
        private GaussianSplatAquaDataSource[] m_validDataSources;
        private GaussianSplatAquaDataSource[] m_prevValidDataSources;
        ...
        private bool AreDataSourcesDirty()
        {
            ...
            for (uint dataSourceIndex = 0; dataSourceIndex < m_validDataSources.Length; ++dataSourceIndex)
            {
                if (m_validDataSources[dataSourceIndex] != m_prevValidDataSources[dataSourceIndex] ||
                    m_validDataSources[dataSourceIndex].dirty)
                {
                    return true;
                }
            }
        ...
        private GaussianSplatAquaDataSource[] GetValidDataSources()
        {
            List<GaussianSplatAquaDataSource> validDataSources = new(m_dataSources.Length);
            foreach (GaussianSplatAquaDataSource dataSource in m_dataSources)
            {
                if (dataSource.IsValid())
                {
        ...
        private GaussianSplatAquaDataSource[] GetVisibleDataSourcesSorted(Camera camera)
        {
            s_getVisibleDataSourcesMarker.Begin();

            List<(GaussianSplatAquaDataSource dataSource, float distance)> visibleDataSources = new();
            ...
            foreach (Plane plane in frustumPlanes)
            {
                ...
                Vector3 planeCentreLocal = transform.InverseTransformPoint(planeCentreWorld);
                Vector3 planeNormalLocal = transform.InverseTransformDirection(frustumPlanes[planeIndex].normal);
            ...
            foreach (var dataSource in m_dataSources)
            {
                ...
                if (GeometryUtility.TestPlanesAABB(localPlanes, dataSourceBounds))
                {
                    float distance = Vector3.Distance(camWorldPos, transform.TransformPoint(dataSourceBounds.center));
                    visibleDataSources.Add((dataSource, distance));
                }
            }

            visibleDataSources.Sort((a, b) => a.distance.CompareTo(b.distance));
            GaussianSplatAquaDataSource[] sorted = new GaussianSplatAquaDataSource[visibleDataSources.Count];
            for (int i = 0; i < visibleDataSources.Count; i++)
                sorted[i] = visibleDataSources[i].dataSource;
        ...
        private void CalculatePhysicalBounds()
        {
            ...
            m_physicalBounds = new Bounds(transform.TransformPoint(corners[0]), Vector3.zero);
            for (int cornerIndex = 1; cornerIndex < corners.Length; cornerIndex++)
            {
                m_physicalBounds.Encapsulate(transform.TransformPoint(corners[cornerIndex]));
            }
        }
        ...
        public void Render(Camera camera, CommandBuffer commandBuffer)
        {
            using (s_populateCommandBufferMarker.Auto())
            {
                ...
                if (m_validDataSources.Length > 0)
                {
                    ...
                    {
                        m_gsRenderer.Run(commandBuffer, camera, transform);
                    }
